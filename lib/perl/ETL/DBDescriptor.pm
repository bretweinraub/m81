...
=head1 NAME - DBDescriptor

DB Descriptor for ETL
...
=item DBDescriptor->new()

initializes on object of type DBDescriptor
...
sub new {
    my DBDescriptor $this = shift;

    print STDERR "in DBDescriptor::new(" . join (",", @_) . ")\n" if $ENV{DEBUG};
    Confess "Missing the value for an argument (even nulls) on creation of DBDescriptor" if scalar @_ % 2 != 0;
    ...
    foreach my $key (keys(%args)) {
        ...
        if (my $fn = $this->UNIVERSAL::can($setterName)) {  # test that the object can call this function
            ...
            Confess "Field named \"$key\" is not defined in object DBDescriptor. typo ?\n";
        }
    }
    ...
    eval {DBDescriptor::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?DBDescriptor::__new/;
    ...
    eval {DBDescriptor::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?DBDescriptor::_new/;
...
=head3 $DBDescriptor->setDebug (debug => "any string")
...
=cut
    ...
    Confess "DBDescriptor->_new requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
