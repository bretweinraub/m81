...
=item ETLDescriptor->new()

initializes on object of type ETLDescriptor
...
sub new {
    my ETLDescriptor $this = shift;

    print STDERR "in ETLDescriptor::new(" . join (",", @_) . ")\n" if $ENV{DEBUG};
    Confess "Missing the value for an argument (even nulls) on creation of ETLDescriptor" if scalar @_ % 2 != 0;
    ...
    foreach my $key (keys(%args)) {
        ...
        if (my $fn = $this->UNIVERSAL::can($setterName)) {  # test that the object can call this function
            ...
            Confess "Field named \"$key\" is not defined in object ETLDescriptor. typo ?\n";
        }
    }
    ...
    eval {ETLDescriptor::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?ETLDescriptor::__new/;
    ...
    eval {ETLDescriptor::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?ETLDescriptor::_new/;
...
=head3 $ETLDescriptor->setDebug (debug => "any string")
...
=cut
    ...
    Confess "ETLDescriptor->_new requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
