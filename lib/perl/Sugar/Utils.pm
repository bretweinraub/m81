...
=head1 NAME - Sugar::Utils

Utilities for working with sugarCRM
...
=item Sugar::Utils->new()

initializes on object of type Sugar::Utils
...
sub new {
    my Sugar::Utils $this = shift;

    print STDERR "in Sugar::Utils::new(" . join (",", @_) . ")\n" if $ENV{DEBUG};
    Confess "Missing the value for an argument (even nulls) on creation of Sugar::Utils" if scalar @_ % 2 != 0;
    ...
    foreach my $key (keys(%args)) {
        ...
        if (my $fn = $this->UNIVERSAL::can($setterName)) {  # test that the object can call this function
            ...
            Confess "Field named \"$key\" is not defined in object Sugar::Utils. typo ?\n";
        }
    }
    ...
    eval {Sugar::Utils::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?Sugar::Utils::__new/;
    ...
    eval {Sugar::Utils::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?Sugar::Utils::_new/;
...
=cut
    ...
    Confess "Sugar::Utils->_new requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
...
=head3 $Utils->SugarKey (data => "any string"*)

 - data ("any string")		 : data to process

generate a unique sugar key based on some input data

=cut

    Confess "Sugar::Utils::SugarKey requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
    my $data = $args{data};
    Confess "argument 'data' is required for Sugar::Utils::SugarKey()" unless defined $data;
