...
=head1 NAME - M80Repository::Base

M80Repository::Base description; stub description please expand
...
=item M80Repository::Base->new()

initializes on object of type M80Repository::Base
...
sub new {
    my M80Repository::Base $this = shift;

    print STDERR "in M80Repository::Base::new(" . join (",", @_) . ")\n" if $ENV{DEBUG};
    Confess "Missing the value for an argument (even nulls) on creation of M80Repository::Base" if scalar @_ % 2 != 0;
    ...
    foreach my $key (keys(%args)) {
        ...
        if (my $fn = $this->UNIVERSAL::can($setterName)) {  # test that the object can call this function
            ...
            Confess "Field named \"$key\" is not defined in object M80Repository::Base. typo ?\n";
        }
    }
    ...
    eval {M80Repository::Base::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?M80Repository::Base::__new/;
    ...
    eval {M80Repository::Base::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?M80Repository::Base::_new/;
...
=head3 $Base->setDebug (debug => "any string")
...
=cut
    ...
    Confess "M80Repository::Base->_new requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
...
=head3 $Base->dump (fields => ARRAY)

 - fields (ARRAY)		 : list of field names to be unwrapped into the object

unwraps the arguments into a traditional m80 repository model

=cut

    my $this = shift;

    Confess "M80Repository::Base->dump requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
    my $fields = $args{fields};
        if (defined $fields) { Confess "argument 'fields' of method M80Repository::Base->dump() is required to be of reference type ARRAY " unless ref($fields) =~ /^ARRAY/;}
