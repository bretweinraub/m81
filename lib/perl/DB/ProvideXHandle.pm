...
=head1 NAME - DB::ProvideXHandle

DB::ProvideXHandle description; stub description please expand
...
DB::ProvideXHandle extends class DB::DBHandle ; refer to the documentation for that object for member variables and methods.
...
=item DB::ProvideXHandle->new()

initializes on object of type DB::ProvideXHandle
...
sub new {
    my DB::ProvideXHandle $this = shift;

    print STDERR "in DB::ProvideXHandle::new(" . join (",", @_) . ")\n" if $ENV{DEBUG};
    Confess "Missing the value for an argument (even nulls) on creation of DB::ProvideXHandle" if scalar @_ % 2 != 0;
    ...
    foreach my $key (keys(%args)) {
        ...
        if (my $fn = $this->UNIVERSAL::can($setterName)) {  # test that the object can call this function
            ...
            Confess "Field named \"$key\" is not defined in object DB::ProvideXHandle. typo ?\n";
        }
    }
    ...
    eval {DB::ProvideXHandle::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?DB::ProvideXHandle::__new/;
    ...
    eval {DB::ProvideXHandle::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?DB::ProvideXHandle::_new/;
...
=head3 $ProvideXHandle->setDebug (debug => "any string")
...
=cut
    ...
    Confess "DB::ProvideXHandle->_new requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
...
=head3 $ProvideXHandle->DBIPrecisionToGenericPrecision (data => "any string"*)
...
=cut
    ...
    Confess "DB::ProvideXHandle->DBIPrecisionToGenericPrecision requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
    my $data = $args{data};
    Confess "argument 'data' is required for DB::ProvideXHandle->DBIPrecisionToGenericPrecision()" unless exists $args{data};
