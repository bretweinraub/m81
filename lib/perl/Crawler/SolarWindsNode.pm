...
=head1 NAME - Crawler::SolarWindsNode

Crawler::SolarWindsNode description; stub description please expand
...
Crawler::SolarWindsNode extends class Crawler::CrawlerBase ; refer to the documentation for that object for member variables and methods.
...
=item Crawler::SolarWindsNode->new()

initializes on object of type Crawler::SolarWindsNode
...
sub new {
    my Crawler::SolarWindsNode $this = shift;

    print STDERR "in Crawler::SolarWindsNode::new(" . join (",", @_) . ")\n" if $ENV{DEBUG};
    Confess "Missing the value for an argument (even nulls) on creation of Crawler::SolarWindsNode" if scalar @_ % 2 != 0;
    ...
    foreach my $key (keys(%args)) {
        ...
        if (my $fn = $this->UNIVERSAL::can($setterName)) {  # test that the object can call this function
            ...
            Confess "Field named \"$key\" is not defined in object Crawler::SolarWindsNode. typo ?\n";
        }
    }
    ...
    eval {Crawler::SolarWindsNode::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?Crawler::SolarWindsNode::__new/;
    ...
    eval {Crawler::SolarWindsNode::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?Crawler::SolarWindsNode::_new/;
...
=head3 $SolarWindsNode->setDebug (debug => "any string")
...
=cut
    ...
    Confess "Crawler::SolarWindsNode->_new requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
...
=cut
    ...
    Confess "Crawler::SolarWindsNode->login requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
