...
$ret .= '
    ...
    eval {' . $this->{name} . '::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?' . $this->{name} . '::__new/;
    ...
    eval {' . $this->{name} . '::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?' . $this->{name} . '::_new/;
...
sub dumpContract {
    ...
    Confess "no contract defined for method $method of $this->{name}.  Maybe you want to be calling the contract() method instead?" unless $methodObject;

    my $description = $methodObject->{description};
    my $static = $methodObject->{static};
...
#    $optionValidationString .= "    eval { \n";
	...
	$optionDescrString .= "\n - $option->{name} ($optionType)$tab : $option->{description}";
        ...
        if ($option->{default}) {
            ...
            if ($positionalParameters) {
                ...
                $optionValidationString .= "    my \$$option->{name} = (scalar \@_ ? shift : " . ($option->{quotedefault} ? "\"" : "") . $default . ($option->{quotedefault} ? "\"" : "") . ");\n";
            } else {

                $optionValidationString .= "    my \$$option->{name} = (\$args{$option->{name}} ? \$args{$option->{name}} : " . ($option->{quotedefault} ? "\"" : "") . $default . ($option->{quotedefault} ? "\"" : "") . ");\n";
            }
        } else {
	...
	if ($option->{required}) {
	    $optionValidationString .= "    Confess \"argument '$option->{name}' is required for $this->{name}".($static ? "::" : "->") . "$method"."()\" " .
		(($positionalParameters || $static)
	...
	if ($option->{ref} =~ /^REF/ or
	    ...
	    $option->{ref} =~ /^HASH/ or
	    $option->{ref} =~ /^ARRAY/) {
	    $optionValidationString .= "        if (defined \$$option->{name}) { Confess \"argument '$option->{name}' of method $this->{name}".($static ? "::" : "->") . "$method"."() is required to be of reference type $option->{ref} \" unless ref(\$$option->{name}) =~ /^$option->{ref}\/;}\n";	    
	} elsif ($option->{ref}) {
	    ...
	    $optionValidationString .= "    if (defined \$$option->{name}) { Confess \"argument '$option->{name}' of method $this->{name}".($static ? "::" : "->") . "$method"."() is required to be of reference type $option->{ref}, but it looks to be of type \" . ref (\$$option->{name})  unless \$$option->{name}"."->isa(\"$option->{ref}\");}\n";
	} elsif ($option->{format}) {
	    $optionValidationString .= "        if (defined \$$option->{name}) { Confess \"argument '$option->{name}' of method $this->{name}".($static ? "::" : "->") . "$method"."() failed format validation of \'" . $option->{format} . "\'\" unless \$$option->{name} =~ /$option->{format}\/;}\n";
	}
...
";
	...
	($positionalParameters 
	 ? ""
	 : '    Confess "' . $this->{name} . ($static ? "::" : "->") . $method . ' requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;' );
	
    $ret .= '
    my $this = shift;
' unless $static;
    $ret .= '    $this->trace( ' . $this->{name} . ' );' . "\n" if $this->{traceOn};
    $ret .= '
'.$argumentProcessor.'
'.$optionValidationString.'
';
...
#
# import the following variables from the current object
#

    my $vars = $methodObject->{vars};   # get the options structure

    foreach my $var (@{$vars}) {
	my $function  = ucFirstChar($var);
	$ret .= "
    my \$$var = \$this->get$function(); ";
    }
