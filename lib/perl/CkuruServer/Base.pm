...
=head1 NAME - CkuruServer::Base

CkuruServer::Base description; stub description please expand
...
=item CkuruServer::Base->new()

initializes on object of type CkuruServer::Base
...
sub new {
    my CkuruServer::Base $this = shift;

    print STDERR "in CkuruServer::Base::new(" . join (",", @_) . ")\n" if $ENV{DEBUG};
    Confess "Missing the value for an argument (even nulls) on creation of CkuruServer::Base" if scalar @_ % 2 != 0;
    ...
    foreach my $key (keys(%args)) {
        ...
        if (my $fn = $this->UNIVERSAL::can($setterName)) {  # test that the object can call this function
            ...
            Confess "Field named \"$key\" is not defined in object CkuruServer::Base. typo ?\n";
        }
    }
    ...
    eval {CkuruServer::Base::__new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?CkuruServer::Base::__new/;
    ...
    eval {CkuruServer::Base::_new($this,%args);};

    croak $@ if $@
	and $@ !~ /^Undefined subroutine.+?CkuruServer::Base::_new/;
...
=cut
    ...
    Confess "CkuruServer::Base->_new requires named arguments, or maybe a non-static method is being called in a static context " if scalar @_ && scalar @_ % 2 != 0;
    my %args = @_;
