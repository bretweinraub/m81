...
my $moduleName;
$moduleName = $ENV{moduleName} if $ENV{moduleName};
my $patchLevel;
$patchLevel = $ENV{patchLevel} if $ENV{patchLevel};
$trace = "0";
...
GetOptions( 	'moduleName'	=> \$moduleName,
...
print_usage() if $help;
unless ($moduleName) { Warn "moduleName is required" ; print_usage() ; } 
...
=item [REQUIRED] 'moduleName'

describes the moduleName to be upgraded


=item 'patchLevel'

migrate database to the described patchlevel
