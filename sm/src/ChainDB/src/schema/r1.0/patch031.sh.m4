...
buildDate="06/14/2006 18:21:19 MDT"
buildUser=bweinrau
buildHost=usbohp380-7
sourceCode=/usr/local/home/bweinrau/dev/sandbox/bweinrau/perf/perfsi_prod/etl/StateMachine/ChainDB/src/schema/src/killed.sql
...
ignoreException([alter table action drop constraint action_ckc1],-2443)


m4_define(states, ['new','analyzing', 'waiting', 'starting', 'started','running', 'succeeded','failed','recovering', 'finished','retry','error','queued','cancel','canceled','killed'])

addCheckConstraint(task,((status, , (states))))
addCheckConstraint(action,((actionstatus, , (states))))
